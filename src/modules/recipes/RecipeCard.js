...
export const SelectedRecipeFooter = ({
  ...
  <Flex backgroundColor="primary_600" justifyContent="space-between" alignItems="center">
    <SelectionButton onClick={() => handleRemoveRecipe(recipeId)} title="Decrease quantity" disabled={selected <= 0}>
      <IconMinusCircle />
    </SelectionButton>
    <Box color="white">
    ...
    <SelectionButton onClick={() => handleAddRecipe(recipeId)} title="Increase quantity" disabled={ maxRecipesSelected || (selectionLimit && selected >= selectionLimit)}>
      <IconPlusCircle />
    </SelectionButton>
  </Flex>
);
